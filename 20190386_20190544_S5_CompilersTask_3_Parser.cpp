...
// Mostafa Mahmoud Anwar Morsy Sadek , ID: 20190544, Group: CS-S5, Program: CS
...
TreeNode* WriteStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    {
        pci->debug_file.Out("Error: Write statement must be an integer expression");
        printf("Error, an \"Write\" should be followed by an integer\n");
        throw -1;
    }
...
TreeNode* AssignStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    if(tree->child[0]->expr_data_type!=INTEGER)
    {
        pci->debug_file.Out("Error, an \"Assign\" should be followed by an integer");
        printf("Error, an \"Assign\" should be followed by an integer\n");
        throw -1;
    }
...
TreeNode* RepeatStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    {
        pci->debug_file.Out("Error, an \"Until\" should be followed by a boolean");
        printf("Error, an \"Until\" should be followed by a boolean\n");
        throw -1;
    }
...
TreeNode* IfStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    {
        pci->debug_file.Out("Error, an \"If\" should be followed by a boolean");
...
void Analyze(TreeNode* node, SymbolTable &st)
{
    ...
    int i;
    for(i=0;i<MAX_CHILDREN;i++) Analyze(node->child[i], st);
    Analyze(node->sibling, st);
}

int SimulateIntNode(TreeNode* node, SymbolTable &st, int* mem)
{
    ...
    else if(node->node_kind==ID_NODE)
    {
        ...
        if(!vi) return 0;
        return mem[vi->memloc];
    }
    else if(node->node_kind==OPER_NODE)
    {
        int l = SimulateIntNode(node->child[0], st, mem);
        int r = SimulateIntNode(node->child[1], st, mem);
        if(node->oper==PLUS) return l+r;
        else if(node->oper==MINUS) return l-r;
        else if(node->oper==TIMES) return l*r;
        else if(node->oper==DIVIDE) return l/r;
        else if(node->oper==POWER) return (int)pow((double)l, (double)r);
    }
    return 0;
}

bool SimulateBoolNode(TreeNode* node, SymbolTable &st, int* mem)
{
    if(node->oper==LESS_THAN)
        return SimulateIntNode(node->child[0], st, mem) < SimulateIntNode(node->child[1], st, mem);
    else if(node->oper==EQUAL)
        return SimulateIntNode(node->child[0], st, mem) == SimulateIntNode(node->child[1], st, mem);
    else {
        printf("Error, can't simulate an oper node\n");
        throw -1;
    }
}

void Simulate(TreeNode* node, SymbolTable st, int* mem)
{
    if (!node) return;

    if(node->node_kind == IF_NODE) {
        if (SimulateBoolNode(node->child[0], st, mem)) {
            Simulate(node->child[1], st, mem);
        } else {
            Simulate(node->child[2], st, mem);
        }
    }
    else if(node->node_kind == REPEAT_NODE) {
        while(true) {
            Simulate(node->child[0], st, mem);
            if (SimulateBoolNode(node->child[1], st, mem)) break;
        }
    }
    else if(node->node_kind == ASSIGN_NODE) {
        VariableInfo* vi=st.Find(node->id);
        if(!vi) return;
        mem[vi->memloc] = SimulateIntNode(node->child[0], st, mem);
    }
    else if(node->node_kind == READ_NODE) {
        VariableInfo* vi=st.Find(node->id);
        if(!vi) return;
        printf("Enter %s: ", node->id);
        scanf("%d", &mem[vi->memloc]);
    }
    else if(node->node_kind == WRITE_NODE) {
        printf("val: %d", SimulateIntNode(node->child[0], st, mem));
    }

    Simulate(node->sibling, st, mem);
}
...
int main()
{
    ...
    CompilerInfo compilerInfo(path, nullptr, "debug.txt");
    TreeNode* tree = Parse(&compilerInfo);
    ...
    int* mem = new int[st.num_vars];
    memset(mem, 0, sizeof(int) * st.num_vars);

    Simulate(tree, st, mem);
